...
static int _iotx_dynamic_activate(iotx_mqtt_region_types_t region, iotx_dev_meta_info_t *meta_info)
{
    char device_secret[IOTX_DEVICE_SECRET_LEN + 1] = {0};
    char kv_key[IOTX_DEVICE_KEY_LEN + DYNAMIC_REG_KV_PREFIX_LEN] = DYNAMIC_REG_KV_PREFIX;
    int res = -1;

    memcpy(kv_key + strlen(kv_key), meta_info->device_key, strlen(meta_info->device_key));
    ...
    if (strlen(device_secret) == 0) {
        ...
        res = IOT_Dynamic_Activate(region, meta_info);
        if (res < STATE_SUCCESS) {
            return res;
        }

        IOT_Ioctl(IOTX_IOCTL_SET_DEVICE_SECRET, meta_info->device_secret);
        callback = iotx_event_callback(ITE_DYNAMIC_ACTIVATE_DEVICE_SECRET);
        if (callback) {
...
void *IOT_MQTT_Construct(iotx_mqtt_param_t *pInitParams)
{
    ...
    /* get dynamic option */
    IOT_Ioctl(IOTX_IOCTL_GET_DYNAMIC_ACTIVATE, (void *)&dynamic);
    ...
    IOT_Ioctl(IOTX_IOCTL_GET_DEVICE_KEY, meta_info.device_key);
...
#ifdef DYNAMIC_ACTIVATE /* get device secret through https dynamic register */
    if (dynamic) {
        ...
        ret = _iotx_dynamic_activate(region, &meta_info);
    ...
    ret = IOT_Sign_MQTT(&meta_info, &g_default_sign);
    if (ret < STATE_SUCCESS) {
