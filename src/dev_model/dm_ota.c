...
int dm_ota_init(void)
{
    ...
    IOT_Ioctl(IOTX_IOCTL_GET_DEVICE_KEY, ctx->device_key);
...
int dm_ota_sub(void)
{
    ...
    /* Init OTA Handle */
    handle = IOT_OTA_Init(ctx->product_key, ctx->device_key, NULL);
    if (handle == NULL) {
...
int dm_ota_switch_device(int devid)
{
    ...
    char dn[IOTX_DEVICE_KEY_LEN + 1] = {0};
    ...
    memcpy(ctx->device_key, dn, strlen(dn) + 1);
    res = dm_ota_sub();
    return res;
}
#endif
