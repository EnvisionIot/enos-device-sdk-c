...
static void user_sample_query_attribute(void)
{
    int res = 0;

    char attribute_payload[50] = {0};
    HAL_Snprintf(attribute_payload, sizeof(attribute_payload), "{\"attributes\":[\"attr_int\"]}");
...
static void user_sample_update_attribute(void)
{
    static int cnt = 1;
    int res = 0;
    char attribute_payload[50] = {0};
    HAL_Snprintf(attribute_payload, sizeof(attribute_payload), "{\"attributes\":{\"attr_int\": %d}}", cnt++);
...
static void user_sample_delete_attribute(void)
{
    int res = 0;
    char attribute_payload[50] = {0};
    HAL_Snprintf(attribute_payload, sizeof(attribute_payload), "{\"attributes\":[\"attr_int\"]}");
...
int main(int argc, char **argv)
{
    ...
    user_sample_update_attribute();
    IOT_EnOS_Yield(EXAMPLE_YIELD_TIMEOUT_MS);
    if (response_id == request_id && response_code == 200)
    {
        EXAMPLE_TRACE("******user_sample_update_attribute success, request_id: %d, response_id: %d, response_code: %d******\n",
                      request_id, response_id, response_code);
    }
    else
    {
        EXAMPLE_TRACE("******user_sample_update_attribute failed******\n");
    }

    user_sample_query_attribute();
    IOT_EnOS_Yield(EXAMPLE_YIELD_TIMEOUT_MS);
    if (response_id == request_id && response_code == 200)
    {
        EXAMPLE_TRACE("******user_sample_query_attribute success, request_id: %d, response_id: %d, response_code: %d******\n",
                      request_id, response_id, response_code);
    }
    else
    {
        EXAMPLE_TRACE("******user_sample_query_attribute failed******\n");
    }

    user_sample_delete_attribute();
    IOT_EnOS_Yield(EXAMPLE_YIELD_TIMEOUT_MS);
    if (response_id == request_id && response_code == 200)
    {
        EXAMPLE_TRACE("******user_sample_delete_attribute success, request_id: %d, response_id: %d, response_code: %d******\n",
                      request_id, response_id, response_code);
    }
    else
    {
        EXAMPLE_TRACE("******user_sample_delete_attribute failed******\n");
    }
