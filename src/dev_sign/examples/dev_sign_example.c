...
#define EXAMPLE_PRODUCT_KEY     "ZTUTEcNf"
#define EXAMPLE_PRODUCT_SECRET  "FV2YOdkPWep"
#define EXAMPLE_DEVICE_KEY     "oBLcN1Ruj2"
#define EXAMPLE_DEVICE_SECRET   "HTtewna5Gih31UFRVzCX"
...
int main(int argc, char *argv[])
{
    ...
    memcpy(meta.device_key, EXAMPLE_DEVICE_KEY, strlen(EXAMPLE_DEVICE_KEY));
    memcpy(meta.device_secret, EXAMPLE_DEVICE_SECRET, strlen(EXAMPLE_DEVICE_SECRET));
    printf("device information init");

    if (IOT_Sign_MQTT(&meta, &sign_mqtt) < 0) {
        return -1;
    }
