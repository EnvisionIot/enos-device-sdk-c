...
#include "wrappers_os.h"
    ...
    char _device_name[IOTX_DEVICE_KEY_LEN + 1]       = "example1";
    ...
    #else
        char _product_key[IOTX_PRODUCT_KEY_LEN + 1]       = "a1Uig3XLRiz";
        char _product_secret[IOTX_PRODUCT_SECRET_LEN + 1] = "oMLYj7mXEhUnNdlx";
        char _device_name[IOTX_DEVICE_KEY_LEN + 1]       = "test_01";
        char _device_secret[IOTX_DEVICE_SECRET_LEN + 1]   = "HKmlugSM6AklMJh4zcbunuKC2K2goxqB";
    #endif
#endif
...
#define otafilename "/tmp/ota.bin"
...
int HAL_Config_Write(const char *buffer, int length)
{
    ...
    snprintf(filepath, sizeof(filepath), "./%s", "conf");
    fp = fopen(filepath, "w");
    if (!fp) {
        return -1;
    }
