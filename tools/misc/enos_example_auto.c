...
#define DEVICE_KEY      "auto_example1"
...
static int user_connected_event_handler(void)
{
    ...
    EXAMPLE_TRACE("EnOS Cloud Connected Successfully!");
...
static int user_measurepoint_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    res = IOT_EnOS_Report(user_example_ctx->master_devid,
...
void user_post_measurepoint(void)
{
    int res = 0;
    user_example_ctx_t *user_example_ctx = user_example_get_ctx();
    ...
    /* Normal Example */
    measurepoint_payload = "{\" ###DM_PROP_ID### \":0}";

    res = IOT_EnOS_Report(user_example_ctx->master_devid,
                             ITM_MSG_POST_MEASUREPOINT,
                             (unsigned char *)measurepoint_payload,
                             strlen(measurepoint_payload));
...
void user_post_event(void)
{
    ...
    res = IOT_EnOS_TriggerEvent(user_example_ctx->master_devid,
                                   event_id,
                                   strlen(event_id),
                                   event_payload,
...
void set_iotx_info()
{
    ...
    IOT_Ioctl(IOTX_IOCTL_SET_DEVICE_KEY, DEVICE_KEY);
...
int main(void)
{
    ...
    memcpy(master_meta_info.device_key, DEVICE_KEY, strlen(DEVICE_KEY));
    ...
    while (1) {
        ...
        if (loop_cnt % 100 == 0 && user_master_dev_available()) {
            EXAMPLE_TRACE("loop_cnt = %d, going to post measurepoint\n", loop_cnt);
            user_post_measurepoint();
        }
        /* Post Event Example */
